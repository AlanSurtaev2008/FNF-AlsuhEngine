...
class ChartingState extends MusicBeatState
{
	...
	public static var curQuant:Int = 0;
	...
	override function create():Void
	{
		...
		quant = new AttachedSprite('ui/chart_quant', 'chart_quant');
	...
	function addAssets():Void
	{
		...
		var player1DropDown = new FlxUIDropDownMenuCustom(10, 25, FlxUIDropDownMenuCustom.makeStrIdLabelArray(characters, true), function(character:String)
		{
		...
		var player2DropDown = new FlxUIDropDownMenuCustom(170, player1DropDown.y, FlxUIDropDownMenuCustom.makeStrIdLabelArray(characters, true), function(character:String)
		{
		...
		var gfVersionDropDown = new FlxUIDropDownMenuCustom(10, player2DropDown.y + 50, FlxUIDropDownMenuCustom.makeStrIdLabelArray(characters, true), function(character:String)
		{
		...
		var stageDropDown = new FlxUIDropDownMenuCustom(170, gfVersionDropDown.y, FlxUIDropDownMenuCustom.makeStrIdLabelArray(stages, true), function(character:String)
		{
		...
		noteSkinInputText = new FlxUIInputText(10, stageDropDown.y + 30, 150, skin, 8);
		blockPressWhileTypingOn.push(noteSkinInputText);
	
		noteSplashesInputText = new FlxUIInputText(10, noteSkinInputText.y + 20, 150, _song.splashSkin, 8);
		blockPressWhileTypingOn.push(noteSplashesInputText);

		var reloadNotesButton:FlxButton = new FlxButton(10, 150, 'Change Notes', function()
		{
		...
		tab_group_assets.add(new FlxText(noteSkinInputText.width + 17.5, noteSkinInputText.y, 0, 'Note Texture'));
		tab_group_assets.add(noteSplashesInputText);
		tab_group_assets.add(new FlxText(noteSplashesInputText.width + 17.5, noteSplashesInputText.y, 0, 'Note Splashes Texture'));
	...
	{
		UI_songIDTitle = new FlxUIInputText(10, 10, 70, _song.songID, 8);
		...
		var saveButton:FlxButton = new FlxButton(200, 8, "Save JSON", function()
		{
			saveLevel();
		});

		var reloadSongJson:FlxButton = new FlxButton(200, saveButton.y + 30, "Reload JSON", function()
		{
		...
		var clear_events:FlxButton = new FlxButton(200, loadEventJson.y + 30, 'Clear events', function()
		{
			clearEvents();
		});
		...
		var reloadSong:FlxButton = new FlxButton(200, clear_notes.y + 30, "Reload Audio", function()
		{
		...
		var difficultyDropDown = new FlxUIDropDownMenuCustom(170, reloadSong.y + 60, FlxUIDropDownMenuCustom.makeStrIdLabelArray(PlayState.difficulties[0], false), function(difficulty:String)
		{
			PlayState.lastDifficulty = CoolUtil.getDifficultyID(difficulty, false, PlayState.difficulties.copy());
		});
		...
		var check_voices = new FlxUICheckBox(10, restart.y + 37, null, null, "Has voice track", 100);
		...
		var check_mute_inst = new FlxUICheckBox(10, check_voices.y + 36.5, null, null, "Mute Instrumental (in editor)", 100);
		...
		var shiftNoteButton:FlxButton = new FlxButton(10, 350, "Shift", function()
		{
	...
	function addSectionUI():Void
	{
		...
		check_mustHitSection = new FlxUICheckBox(10, 35, null, null, "Camera Points to P1?", 100);
		check_mustHitSection.name = 'check_mustHit';
		check_mustHitSection.checked = _song.notes[curSection].mustHitSection;

		check_gfSection = new FlxUICheckBox(130, 35, null, null, "GF Section", 100);
		...
		tab_group_section.add(new FlxText(74, 110, 'Section\'s BPM'));
	...
	function addChartingUI():Void
	{
		...
		if (FlxG.save.data.vortex == null) FlxG.save.data.vortex = false;
		check_vortex.checked = FlxG.save.data.vortex;
		vortex = FlxG.save.data.vortex;
		check_vortex.callback = function()
		{
			FlxG.save.data.vortex = check_vortex.checked;
			vortex = FlxG.save.data.vortex;
			reloadGridLayer();
		};
		...
		metronomeOffsetStepper = new FlxUINumericStepper(metronomeStepper.x + 150, metronomeStepper.y, 25, 0, 0, 1000, 1);
		...
		voicesVolume = new FlxUINumericStepper(instVolume.x + 150, instVolume.y, 0.1, 1, 0, 1, 1);
	...
	override function update(elapsed:Float):Void
	{
		...
		_song.songID = UI_songIDTitle.text;
	...
	function reloadGridLayer():Void
	{
		...
		var gridBlack:FlxSprite = new FlxSprite(0, gridBG.height / 2);
		gridBlack.makeGraphic(Std.int(GRID_SIZE * 9), Std.int(gridBG.height / 2), FlxColor.BLACK);
		gridBlack.alpha = 0.4;
		gridLayer.add(gridBlack);

		var gridBlackLine:FlxSprite = new FlxSprite(gridBG.x + gridBG.width - (GRID_SIZE * 4));
		gridBlackLine.makeGraphic(2, Std.int(gridBG.height), FlxColor.BLACK);
		gridLayer.add(gridBlackLine);

		for (i in 1...4)
		{
			var beatsep1:FlxSprite = new FlxSprite(gridBG.x, (GRID_SIZE * (4 * curZoom)) * i);
			beatsep1.makeGraphic(Std.int(gridBG.width), 1, 0x44FF0000);
	...
	function updateWaveform():Void
	{
		...
		var checkForVoices:Int = waveformUseInstrumental.checked ? 0 : 1;
	...
	private function newSection(lengthInSteps:Int = 16, mustHitSection:Bool = true, altAnim:Bool = false, gfSection:Bool = false):SwagSection
	{
		var sec:SwagSection = {
			lengthInSteps: lengthInSteps,
			bpm: _song.bpm,
	...
	{
		PlayState.SONG = Song.loadFromJson(song.toLowerCase() + CoolUtil.getDifficultySuffix(PlayState.storyDifficulty, PlayState.difficulties), song.toLowerCase());
	...
	private function saveLevel():Void
	{
		...
		if ((data != null) && (data.length > 0))
		{
			...
			_file.save(data.trim(), _song.songID.toLowerCase() + CoolUtil.getDifficultySuffix(PlayState.storyDifficulty, PlayState.difficulties) + ".json");
		}
	}
	...
	function shiftNotes(measure:Int = 0, step:Int = 0, ms:Int = 0):Void
	{
		...
		{
			newSong.push(newSection(16,_song.notes[daSection1].mustHitSection,_song.notes[daSection1].altAnim,_song.notes[daSection1].gfSection));
		}
