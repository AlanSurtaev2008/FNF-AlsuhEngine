...
class FlxUIDropDownMenuCustom extends FlxUIGroup implements IFlxUIWidget implements IFlxUIClickable implements IHasParams // forked from Psych Engine
{
	...
	public function new(X:Float = 0, Y:Float = 0, DataList:Array<StrNameLabel>, ?Callback:String->Void, ?Header:FlxUIDropDownHeader,
			?DropPanel:FlxUI9SliceSprite, ?ButtonList:Array<FlxUIButton>, ?UIControlCallback:Bool->FlxUIDropDownMenuCustom->Void):Void
	{
		super(X, Y);
	...
	function mouseOverlapping():Bool
	{
		...
		if (mousePoint.x >= objPoint.x && mousePoint.y >= objPoint.y && mousePoint.x < objPoint.x + this.width && mousePoint.y < objPoint.y + this.height) {
			return true;
		}
...
class FlxUIDropDownHeader extends FlxUIGroup
{
	 ...
	 */
	public function new(Width:Int = 120, ?Background:FlxSprite, ?Text:FlxUIText, ?Button:FlxUISpriteButton):Void
	{
		super();
